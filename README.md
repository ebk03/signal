...
Signal scrapes Hacker News "Who is hiring?" threads into Postgres, then lets you ask natural-language questions about the data, like "what's the most in-demand skill this month?" or "which companies are hiring for remote React roles?", and watches an LLM agent decide what SQL to write, run it, and pick a chart to visualize the result.
...
This is an npm workspaces monorepo (`client` + `server`), so one install at the root covers both:
...
The agent connects through a dedicated `agent_readonly` Postgres role that can only `SELECT` from `job_postings`, so it's physically incapable of writing, independent of any application-level checks. This script creates it (or rotates its password if it already exists), generating a secure password automatically:
...
Open the frontend URL and you'll land on the public dashboard. Sign up to unlock the agent chat.

## Running tests

**Unit tests** (Jest, `sqlGuard` and scraper parsing/skill extraction):
...
⚠️ The e2e test exercises the *real* Anthropic API and your real database, with no mocking. Each run costs a small amount and takes ~10 seconds. Don't wire it into a CI pipeline that runs on every push without knowing that.

## Deployment

Backend runs on Lambda behind an API Gateway HTTP API (no Docker needed: `aws-cdk-lib`'s `NodejsFunction` bundles with esbuild, which is exactly why `bcryptjs` was chosen over native `bcrypt` back when auth was built). Frontend is a static build on S3 behind CloudFront. Both are provisioned via CDK in `infra/`, added as a third npm workspace.

**Why Lambda over ECS Fargate/App Runner:** no Docker needed at all (this machine didn't have it installed, and CDK's container-based bundling would have required it), the dependency choices already assumed Lambda, and true pay-per-request pricing suits a portfolio project's sporadic traffic. The honest tradeoff: `@fastify/rate-limit`'s in-memory store means the per-user limit is really per-warm-container under concurrent Lambda invocations, not strictly global, which is acceptable for a demo but not for real adversarial traffic.
...
Lambda cold starts after ~5–15 minutes of idle, and the first request re-runs Fastify's full plugin registration and feels noticeably slower. Warm it up right before you demo:
